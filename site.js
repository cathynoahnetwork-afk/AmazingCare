...
(function () {
  ...
  function nav() {
    ...
    drawer.parentNode.insertBefore(scrim, drawer);
    var openD = function () {
    ...
    scrim.addEventListener('click', shutD);
    if (close) close.addEventListener('click', shutD);
    $$('a', drawer).forEach(function (a) { a.addEventListener('click', shutD); });
  ...
  function testimonials() {
    ...
    var show = function (i, dir) {
      ...
      var d = dir || (i > cur ? 'right' : 'left');
      tabs.forEach(function (t, n) { t.classList.toggle('is-active', n === i); });
      panels.forEach(function (p, n) {
        p.classList.remove('tm-from-right', 'tm-from-left');
        if (n === i && i !== cur) { void p.offsetWidth; p.classList.add('tm-from-' + d); }
        p.classList.toggle('is-active', n === i);
      });
      var strip = tabs[i] && tabs[i].parentElement;
      if (strip && strip.scrollWidth > strip.clientWidth) {
        strip.scrollTo({ left: tabs[i].offsetLeft - 20, behavior: 'smooth' });
      }
      cur = i;
    };
    tabs.forEach(function (t, i) { t.addEventListener('click', function () { show(i); }); });
    var zone = panels[0].parentElement, sx = 0, sy = 0, on = false;
    zone.addEventListener('touchstart', function (e) { var t = e.touches[0]; sx = t.clientX; sy = t.clientY; on = true; }, { passive: true });
    zone.addEventListener('touchend', function (e) {
      if (!on) return; on = false;
      var t = e.changedTouches[0], dx = t.clientX - sx, dy = t.clientY - sy;
      if (Math.abs(dx) < 48 || Math.abs(dx) < Math.abs(dy) * 1.3) return;
      if (dx < 0) show(cur + 1, 'right'); else show(cur - 1, 'left');
    }, { passive: true });
    show(0);
  }
  ...
  function video() {
    ...
    var open = function (id) {
      frame.innerHTML = '<iframe src="https://www.youtube.com/embed/' + id +
        '?autoplay=1&rel=0" title="' + ((document.documentElement.lang || '').indexOf('en') === 0 ? 'Interview video' : '\u4e13\u8bbf\u5f71\u7247') + '" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>';
  ...
  function mealStack() {
    ...
    if (dotWrap && !dotWrap.children.length) {
      cards.forEach(function (_, i) {
        ...
        d.type = 'button';
        d.setAttribute('aria-label', (document.documentElement.lang || '').indexOf('en') === 0 ? 'Meal card ' + (i + 1) : '第 ' + (i + 1) + ' 张餐卡');
  ...
  /* ---------- mobile: sticky booking bar + WhatsApp sheet ---------- */
  function mobileBar() {
    var bar = $('[data-m-bar]'), sheet = $('[data-wa-sheet]');
    if (!bar) return;
    var open = function () { if (sheet) { sheet.classList.add('is-open'); document.body.style.overflow = 'hidden'; } };
    var shut = function () { if (sheet) { sheet.classList.remove('is-open'); document.body.style.overflow = ''; } };
    $$('[data-wa-open]').forEach(function (b) { b.addEventListener('click', open); });
    $$('[data-wa-close]').forEach(function (b) { b.addEventListener('click', shut); });
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape') shut(); });
    var hero = $('#top'), foot = $('footer');
    var tick = function () {
      var hide = false;
      if (hero) hide = window.scrollY < hero.offsetHeight * 0.5;
      if (foot && foot.getBoundingClientRect().top < window.innerHeight - 40) hide = true;
  ...
  function init() {
    ...
    panels(); tracks(); carousels(); hotspots(); mealStack(); mobileBar();
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
  else init();

  /* ---------- mobile: after picking a room, bring its details into view ---------- */
  document.addEventListener('click', function (e) {
    var b = e.target.closest && e.target.closest('[data-m-roomlist] > button');
    if (!b || window.innerWidth > 900) return;
    var grid = b.closest('[data-m-stack]');
    var target = grid && grid.children[1];
    if (!target) return;
    setTimeout(function () {
  ...
  /* ---------- mobile: awards carousel dots ---------- */
  (function () {
    var track = document.querySelector('[data-awards]');
    var dots = document.querySelector('[data-awards-dots]');
    if (!track || !dots) return;
    var items = Array.prototype.slice.call(track.children);
    items.forEach(function (_, i) {
      var b = document.createElement('button');
      b.type = 'button';
      b.setAttribute('aria-label', 'Award ' + (i + 1));
    ...
    var sync = function () {
      ...
      items.forEach(function (it, i) { var d = Math.abs(it.offsetLeft + it.clientWidth / 2 - mid); if (d < bd) { bd = d; best = i; } });
      Array.prototype.forEach.call(dots.children, function (b, i) { b.classList.toggle('is-active', i === best); });
    };
    var raf = 0;
  ...
  /* ---------- mobile: tap a photo to enlarge ---------- */
  (function () {
    var mq = window.matchMedia('(max-width: 760px)');
    var box = null, pic = null;
    var ok = function (img) {
      if (!img || img.tagName !== 'IMG') return false;
      if (img.closest('a, button, .site-header, .nav-drawer, footer, .img-zoom, .video-modal')) return false;
    ...
    var build = function () {
      ...
      box.innerHTML = '<button type="button" class="img-zoom-close" aria-label="Close">\u00d7</button><img alt="">';
      pic = box.querySelector('img');
      box.addEventListener('click', function (e) { if (e.target !== pic) close(); });
    ...
    var close = function () {
      ...
      document.documentElement.style.overflow = '';
      setTimeout(function () { box.classList.remove('is-open'); pic.removeAttribute('src'); }, 240);
    };
    ...
    var tag = function () {
      Array.prototype.forEach.call(document.querySelectorAll('main img, section img'), function (img) {
        if (mq.matches && !img.closest('a, button')) img.setAttribute('data-zoomable', ''); else img.removeAttribute('data-zoomable');
      });
    };
    tag();
    if (mq.addEventListener) mq.addEventListener('change', tag);
  })();

  /* ---------- mobile: swipe the meal card ---------- */
  (function () {
    var row = document.querySelector('[data-m-meal-row]');
    if (!row) return;
    var sx = 0, sy = 0, on = false;
    row.addEventListener('touchstart', function (e) { var t = e.touches[0]; sx = t.clientX; sy = t.clientY; on = true; }, { passive: true });
    row.addEventListener('touchend', function (e) {
      if (!on) return; on = false;
      var t = e.changedTouches[0], dx = t.clientX - sx, dy = t.clientY - sy;
      if (Math.abs(dx) < 48 || Math.abs(dx) < Math.abs(dy) * 1.3) return;
      var b = row.querySelector(dx < 0 ? '[data-switch-next]' : '[data-switch-prev]');
      if (b) b.click();
    }, { passive: true });
  })();
})();
